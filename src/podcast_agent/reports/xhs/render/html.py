...
def render_intro_html(*, note: dict[str, Any], width: int, height: int) -> str:
    ...
    source_html = _render_intro_source(frontmatter)
    return _page_html(
        width=width,
        height=height,
        body=f"""
...
def _render_intro_source(frontmatter: dict[str, Any]) -> str:
    author = str(frontmatter.get("author") or "").strip()
    source_title = str(frontmatter.get("source_title") or "").strip()
    source = str(frontmatter.get("source") or "").strip()
    url = str(frontmatter.get("url") or "").strip()
    if not author and not source_title and not source and not url:
        return ""

    lines: list[str] = []
    if author:
        lines.append(_render_source_row(author))
    if source_title:
        lines.append(_render_source_row(source_title))
    if source and not author and not source_title:
        lines.append(_render_source_row(source))
    if url:
        lines.append(_render_source_row(url))
    return f'<div class="intro-source">{"".join(lines)}</div>'


def _render_source_row(value: str) -> str:
    return (
        '<div class="source-row">'
        '<span class="source-label" aria-hidden="true">•</span>'
        f'<span class="source-value">{render_text(value)}</span>'
        "</div>"
    )
...
def _page_html(*, width: int, height: int, body: str, extra_gap: float = 0) -> str:
    ...
    footer = '<footer class="page-footer">Generated by Podcast Agent</footer>'
    return f"<!doctype html><html><head><meta charset=\"utf-8\"><style>{css}</style></head><body>{body}{footer}</body></html>"
