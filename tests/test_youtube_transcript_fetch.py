...
def test_bilibili_transcript_fetcher_uses_configured_downloader(tmp_path: Path, monkeypatch) -> None:
    captured: dict[str, object] = {}

    class FakeBilibiliDownloader(FakeTranscriptDownloader):
    ...
    monkeypatch.setattr(
        "podcast_agent.elements.youtube_transcript.BilibiliYtDlpDownloader",
        FakeBilibiliDownloader,
    )
    monkeypatch.setattr("podcast_agent.elements.youtube_transcript.BILIBILI_COOKIES_FILE", "/tmp/env-bili.txt")
    monkeypatch.setattr("podcast_agent.elements.youtube_transcript.BILIBILI_COOKIES_FROM_BROWSER", "chrome")
    monkeypatch.setattr("podcast_agent.elements.youtube_transcript.BILIBILI_USER_AGENT", "Custom UA")
    ...
    YoutubeTranscriptFetcher(elements_dir=tmp_path).fetch(source)
